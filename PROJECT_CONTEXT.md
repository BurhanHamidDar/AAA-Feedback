...
7. Parent-First Verification Flow: All submissions check phone numbers against the `student_contacts` registry first. If matched, the user proceeds immediately to the Parent welcome flow without entering an admission number. Unregistered numbers go to the Student verification fallback flow.
