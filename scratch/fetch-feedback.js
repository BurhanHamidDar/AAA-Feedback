...
async function run() {
  ...
  if (error) {
    ...
    console.log(`Fetched ${data.length} feedback records:`);
    data.forEach((f, idx) => {
      console.log(`\n--- Record ${idx + 1} ---`);
      console.log(`ID: ${f.id}`);
      console.log(`Tracking: ${f.tracking_number}`);
      console.log(`Raw text: "${f.raw_text}"`);
      console.log(`Summary: "${f.summary}"`);
      console.log(`Category: ${f.category}`);
      console.log(`Sentiment: ${f.sentiment}`);
